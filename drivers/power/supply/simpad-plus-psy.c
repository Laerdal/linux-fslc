...
static int lm_pmu_dt(struct sbp_priv *priv)
{
	...
	priv->gpio_dcin_valid = devm_gpiod_get(dev, "dcin", GPIOD_ASIS);
	if(IS_ERR(priv->gpio_dcin_valid)) {
		dev_err(dev, "%s: Invalid GPIO dcin-gpio\n", __func__);
		return -EINVAL;
	}
	...
	for (n=0; n < num_gpio; n++) {
		// set initial value to high. gpiod handles the physical line level
		// i.e. active low lines will be set low, active high lines will be set high.
		priv->gpio_bat_ce[n] = devm_gpiod_get_index(dev, "bat-ce", n, GPIOD_OUT_HIGH);
	...
	for (n=0; n < num_gpio; n++) {
		...
		if (IS_ERR(priv->gpio_startup_code[n])) {
			dev_err(dev, "%s: Unable to get %s gpio_startup_code[%d]\n", __func__, startup_names[n], n);
			return -EINVAL;
		}
	}
	return 0;
