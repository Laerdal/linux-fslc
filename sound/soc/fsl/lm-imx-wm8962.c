...
static int hpjack_status_check(void *priv)
{
	...
	dev_dbg(&data->pdev->dev, "%s: hpdet = %d\n", __func__, hp_status);
	buf = kmalloc(32, GFP_ATOMIC);
	if (!buf) {
		dev_err(&data->pdev->dev, "%s kmalloc failed\n", __func__);
		return -ENOMEM;
...
static int imx_wm8962_spk_amp_event(struct snd_soc_dapm_widget *w, struct snd_kcontrol *k, int event)
{
	...
	if (!IS_ERR(data->options.spk_amp_gpio)) {
		...
		if (SND_SOC_DAPM_EVENT_ON(event))
			gpiod_set_value(data->options.spk_amp_gpio, 1);
		else
			gpiod_set_value(data->options.spk_amp_gpio, 0);
	}
	return 0;
}
...
static int imx_wm8962_jack_init(struct imx_wm8962_data *data, struct device *dev)
{
	int ret;
	if (!IS_ERR(data->options.hp_gpio)) {
		imx_hp_jack_gpio.gpiod_dev = dev;
		imx_hp_jack_gpio.data = data;
		imx_hp_jack_gpio.name = "hp-det";
	...
	if (!IS_ERR(data->options.mic_gpio)) {
		imx_mic_jack_gpio.gpiod_dev = dev;
		imx_mic_jack_gpio.name = "mic-det";
		imx_mic_jack_gpio.data = data;
		imx_mic_jack_gpio.jack_status_check = micjack_status_check;
		imx_mic_jack_gpio.invert = data->options.mic_active_level;
...
static int imx_wm8962_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(data->options.spk_amp_gpio)) {
		dev_err(&pdev->dev, "%s: Unable to request speaker-amp-gpios for amp\n", __func__);
		goto cleanup;
	}
	...
	ret = imx_wm8962_jack_init(data, &pdev->dev);
cleanup:
	if (cpu_pdev)
